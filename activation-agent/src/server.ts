...
export const server = new Elysia()
  ...
  .post('/thread/:id/pick', async ({ params: { id } }) => {
    ...
    const pickedTweet =
      state.generated_tweets[state.generated_tweets.length - 1].content
    return renderPickedTweetMessage({ thread_id: id, pickedTweet: pickedTweet })
  })
  ...
  .post(
    ...
    async ({ body, params: { id } }) => {
      ...
      if (state.is_verified) {
        ...
        const generated_tweet = state.generated_tweets[state.generated_tweets.length - 1].content
        return renderPickedTweetMessage({ thread_id: id, pickedTweet: generated_tweet })
      }
    },
    {
      body: t.Object({
