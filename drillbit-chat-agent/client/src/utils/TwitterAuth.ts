...
import { resetState } from "../api/API";
...
export function useTwitterAuth() {
  ...
  useEffect(() => {
    const subscription = auth.onAuthStateChanged((user) => {
      setUser(user);
      setThreadId(`${user?.uid + "_" + Date.now()}`);
      setLoading(false);
    });
  ...
  const signOutUser = async () => {
    try {
      await resetState(user.uid, threadId)
      await signOut(auth);
    } catch (error: any) {
      setError(error.message);
      throw error;
  ...
  return useMemo(
    () => ({ user, threadId, loading, error, signInWithTwitter, signOutUser }),
    [user, threadId, loading, error, signInWithTwitter, signOutUser]
  );
}
